...
d_args = read_csv("data/wsj_proportions.csv") %>%
  ...
  #unite("type", 3:4) %>%
  #spread(type, count) %>%
  #select(-obj_passubj, -NA_NA, -`no-obj_no-passsubj`)
...
What do we want to see?
  - X-axis = percent of time it has an argument.
...
d_agg = rbind(d_ngram, d_lstm, d_rnng)
...
ggsave("./images/argstruct-downsample_base-pres.png",height=5,width=5)
...
d_agg %>%
  ...
  ggplot(aes(x=vbd_freq, y=m, ymin=lower, ymax=upper, color=is_trans)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruc-downsamplet_base.png",height=5,width=5)
...
d_obj %>%
  ...
  group_by(model, vb_freq, VB_obj) %>%
    summarise(m = mean(obj_exp),
              s=std.error(obj_exp),
              upper=m+1.96*s,
              lower=m-1.96*s)%>%
  ungroup() %>%
  ggplot(aes(x=VB_obj, y=m, ymin=lower, ymax=upper, color=vb_freq)) +
    geom_point(stat="identity", position="dodge", size=1, alpha=0.8) +
    stat_smooth(method="lm", se=T, formula=y ~ x) +
    geom_errorbar(width=.01, alpha=0.1) +
    geom_hline(yintercept=0, color="blue", alpha=0.5) +
    ylab("Object Expectation") +
    xlab("Frequency") +
    facet_grid(model~vb_freq, scales="free") +
    #ylim(0, 1) +
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruct-downsample-pres.png",height=5,width=12)
```
...
d_obj %>%
  ...
  group_by(model, verb, vbd_freq, VBD_obj) %>%
    summarise(m = mean(obj_exp),
              s=std.error(obj_exp),
              upper=m+1.96*s,
              lower=m-1.96*s)%>%
  ungroup() %>%
  ggplot(aes(x=VBD_obj, y=m, ymin=lower, ymax=upper, color=vbd_freq)) +
    geom_point(stat="identity", position="dodge", size=2, alpha=0.2) +
    stat_smooth(method="lm", se=T, formula=y ~ x) +
    geom_errorbar(width=.01, alpha=0.2) +
    geom_hline(yintercept=0, color="blue", alpha=0.5) +
    ylab("Object Expectation") +
    xlab("Frequency") +
    facet_grid(model~vbd_freq, scales = "free") +
    #ylim(-5, 5) +
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruc-downsample-trans.png",height=5,width=5)

```

```{r}
d_obj %>%
  filter((test == "transf-longmod" | test == "transf-nomod" | test == "transf-mod")) %>%
  filter(total_freq == "2" | total_freq == "3" | total_freq == "5" | total_freq == "10") %>%
  group_by(model, test, verb, total_freq, VBD_obj) %>%
    summarise(m = mean(obj_exp),
              s=std.error(obj_exp),
              upper=m+1.96*s,
              lower=m-1.96*s)%>%
  ungroup() %>%
  ggplot(aes(x=VBD_obj, y=m, ymin=lower, ymax=upper, color=model)) +
    geom_point(stat="identity", position="dodge", size=2, alpha=0.1) +
    stat_smooth(method="lm", se=T, formula=y ~ x, alpha=0.5) +
    geom_errorbar(width=.01, alpha=0.1) +
    geom_hline(yintercept=0, color="blue", alpha=0.5) +
    ylab("Object Expectation") +
    xlab("Frequency") +
    facet_grid(test~total_freq, scales = "free")
    #ylim(-5, 5) +
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruc-downsample-transform.png",height=5,width=12)
...
d_agg %>%
  ...
  ggplot(aes(x=total_freq, y=m, ymin=lower, ymax=upper, color=is_trans)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruct-downsample_transf.png",height=5,width=5)
...
d_agg %>%
  ...
  ggplot(aes(x=vbd_freq, y=m, ymin=lower, ymax=upper, color=is_trans)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruct-downsample_invar.png",height=5,width=5)
...
d_lstm = merge(d, lstm_results, by=0, all=TRUE)
d_rnng = merge(d, rnng_results, by=0, all=TRUE)
d_all = rbind(d_ngram, d_lstm, d_rnng)
...
d_agg %>%
  ...
  ggplot(aes(x=freq_cat, y=m, ymin=lower, ymax=upper, color=pos)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/number-downsample_base.png",height=5,width=5)
...
d_agg %>%
  ...
  ggplot(aes(x=freq_cat, y=m, ymin=lower, ymax=upper, color=pos)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/number-downsample_transf.png",height=5,width=5)
