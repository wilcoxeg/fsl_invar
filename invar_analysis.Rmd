...
results = read.csv("test_items/argstruct_lstm_output.txt", sep=" ", header=FALSE) %>%
  rename("word_1" = V1) %>%
  rename("sent_id" = V2) %>%
  rename("sent_pos" = V3) %>%
  rename("w_len" = V4) %>%
...
d_mod = read.csv("test_items/argstruct-mod_items.csv", header=FALSE) %>%
  rename("word" = V1) %>%
  rename("verb" = V2) %>%
  rename("pos" = V3) %>%
  rename("freq" = V4) %>%
  ...
  rename("item_number" = V9) %>%
  rename("test" = V10) %>%
  mutate(test = if_else(test=="transf", "transf-mod", "base-mod"))

results_mod = read.csv("test_items/argstruct-mod_lstm_output.txt", sep=" ", header=FALSE) %>%
  rename("word_1" = V1) %>%
  rename("sent_id" = V2) %>%
  rename("sent_pos" = V3) %>%
  rename("w_len" = V4) %>%
...
d = rbind(d, d_mod, d_pres)
results = rbind(results, results_mod, results_pres)
...
```{r}
d_agg = merge(d, results, by=0, all=TRUE) %>%
  arrange(as.numeric(Row.names)) %>%
  filter((target_id-1 == sent_pos) | (target_id-2 == sent_pos) | (target_id-3 == sent_pos)) %>%
  filter( !((target_id-3 == sent_pos) & (test=="base")) ) %>%
  group_by(verb, pos, is_trans, is_intrans, gram, test, freq, item_number) %>%
...
d_agg %>%
  ...
  group_by(is_trans, test, vbd_freq) %>%
...
```{r}
d_agg %>%
  filter( test=="transf" | test == "transf-mod") %>%
  group_by(is_trans, test, total_freq) %>%
    summarise(m = mean(obj_exp),
              s=std.error(obj_exp),
              upper=m+1.96*s,
              lower=m-1.96*s)%>%
  ...
  ggplot(aes(x=total_freq, y=m, ymin=lower, ymax=upper, color=is_trans)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruct_transf.png",height=5,width=5)
...
```{r}
d_agg %>%
  filter(VBN == 0, test=="transf" | test == "transf-mod", is_trans != "Ambitrans") %>%
  ...
  ggplot(aes(x=vbd_freq, y=m, ymin=lower, ymax=upper, color=is_trans)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/argstruct_invar.png",height=5,width=5)
...
```{r}
d = read.csv("number_items.csv", header=FALSE) %>%
  rename("word" = V1) %>%
  rename("verb" = V2) %>%
  rename("pos" = V3) %>%
  rename("freq" = V4) %>%
...
results = read.csv("number_lstm_output.txt", sep=" ", header=FALSE) %>%
  rename("word_1" = V1) %>%
  rename("sent_id" = V2) %>%
  rename("sent_pos" = V3) %>%
  rename("w_len" = V4) %>%
...
d_agg = merge(d, results, by=0, all=TRUE) %>%
  arrange(as.numeric(Row.names)) %>%
  filter(target_id-1 == sent_pos) %>%
  select(-Row.names, -word, -target_id, -word_1, -sent_id, -sent_pos, -w_len) %>%
  spread(gram, surprisal) %>%
...
```{r}
d_agg %>%
  filter(test == "base_pp" | test == "base_simple") %>%
  group_by(pos, freq_cat, test) %>%
  ...
  ggplot(aes(x=freq_cat, y=m, ymin=lower, ymax=upper, color=pos)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/number_base.png",height=5,width=5)
...
d_agg %>%
  ...
  ggplot(aes(x=freq_cat, y=m, ymin=lower, ymax=upper, color=pos)) +
    ...
    theme(axis.text=element_text(size=10),
          legend.position = "bottom")
ggsave("./images/number_transf.png",height=5,width=5)
