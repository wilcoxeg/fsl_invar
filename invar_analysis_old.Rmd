...
d_agg %>%
  #filter(vb_freq == "2" | vb_freq == "3" | vb_freq == "5" | vb_freq == "10" | vb_freq == "20" | vb_freq == "30") %>%
  filter(test == "base-pres" & is_trans != "Ambitrans") %>%
  mutate(vb_freq = as.numeric(vb_freq)) %>%
  
  ggplot(aes(x=jitter(vb_freq), y=obj_exp, color=is_trans)) +
    theme_bw() +
    #geom_point(stat="identity", position="dodge", size=2) +
...
d_agg %>%
  #filter(vb_freq == "2" | vb_freq == "3" | vb_freq == "5" | vb_freq == "10" | vb_freq == "20" | vb_freq == "30") %>%
  filter(test == "base-pres",  is_trans != "Ambitrans") %>%
  mutate(vb_freq = as.numeric(as.character(vb_freq))) %>%
  mutate(obj_exp = if_else(obj_exp > 0, 1, 0)) %>%
  mutate(obj_exp = if_else(is_trans == "Intrans", 1-obj_exp, obj_exp)) %>%
  
  ggplot(aes(x=jitter(vb_freq), y=obj_exp)) +
    theme_bw() +
    #geom_point(stat="identity", position="dodge", size=2) +
    stat_smooth(method = 'glm', method.args = list(family = "binomial"), linetype="dashed", size = 0.5) +
    stat_smooth(aes(color=is_trans), method = 'glm', method.args = list(family = "binomial"), size = 0.5) +
    #geom_point() +
    geom_hline(yintercept=0.5, color="blue", alpha=0.5) +
    scale_y_continuous(limits = c(0,1)) +
    ylab("Object Expectation Accuracy") +
    xlab("Frequency") +
    ggtitle("Argument Structure: Active, Present") +
    facet_grid(.~model) +
    theme(axis.title.x=element_blank(),
...
d_agg %>%
  ...
  mutate(obj_exp = if_else(obj_exp > 0, 1, 0)) %>%
  mutate(obj_exp = if_else(is_trans == "Intrans", 1-obj_exp, obj_exp)) %>%
  mutate(vbd_freq = as.numeric(as.character(vbd_freq))) %>%

  ggplot(aes(x=jitter(vbd_freq), y=obj_exp)) +
    theme_bw() +
    #geom_point(stat="identity", position="dodge", size=2) +
    stat_smooth(method = 'glm', method.args = list(family = "binomial"), linetype="dashed", size = 0.5) +
    stat_smooth(aes(color=is_trans), method = 'glm', method.args = list(family = "binomial"), size = 0.5) +
    ...
    theme(axis.title.x=element_blank(),
          legend.position = "none")
ggsave("./images/argstruct-past.png",height=2,width=6)
...
d_agg %>%
  ...
  ggplot(aes(x=jitter(vbd_freq), y=obj_exp)) +
    ...
    ylab("Object Expectation Accuracy") +
    xlab("Frequency") +
    ggtitle("Argument Structure: Transfer (past)") +
    facet_grid(test~model) +
    theme(axis.title.x=element_blank(),
          legend.position = "none")
ggsave("./images/argstruct-transf.png",height=4,width=6)
...
```{r Plural Expectation, eval=False}
d_agg %>%
  drop_na() %>%
...
d_num_base_acc %>%
  group_by(model, freq_cat, test) %>%
    summarise(m = mean(pl_exp),
              n = n(),
              upper=m+binconf(m, n=n())[2],
              lower=m-binconf(m, n=n())[3])%>%
  ungroup() %>%
  mutate(freq_cat = log10(freq_cat)) %>%
  ggplot(aes(x=freq_cat, y=m, ymin=lower, ymax=upper, color = model, fill=model)) +
    theme_bw()+
    geom_point(stat="identity", position="dodge", size=2) +
    geom_errorbar(width=0.001, alpha=0.8) +
    geom_hline(yintercept=0.5, color="blue", alpha=0.5) +
...
exposure_test = function(df){
  m = glm(pl_exp ~ freq_cat, family = binomial, data = df)
...
supervision_test = function(df){
  m1 = glm(pl_exp ~ model, family = binomial, data = df %>% filter(model == "rnng" | model == "lstm") )
  m2 = glm(pl_exp ~ model, family = binomial, data = df %>% filter(model == "actionLstm" | model == "lstm") )
  df = df %>%
    summarise(e_rnng = coef(summary(m1))[2],
              p_rnng = coef(summary(m1))[8],
              e_action = coef(summary(m2))[2],
              p_action = coef(summary(m2))[8])
}
...
d_lm = d_num_base_acc %>%
  filter(test == "base_simple", model == "rnng" | model == "lstm") %>%
  glm(pl_exp ~ model, family = binomial, data = .)
coef(summary(d_lm))
...
d_agg %>%
  ...
  ggplot(aes(x=jitter(freq_cat), y=pl_exp)) +
    theme_bw() +
    geom_hline(yintercept=0.5, color="black", alpha=1, size = 0.1) +
    stat_smooth(aes(color=pos), method = 'glm', method.args = list(family = "binomial"), size = 1) +
    stat_smooth(method = 'glm', method.args = list(family = "binomial"), linetype="dashed", size = 0.7) +
    scale_y_continuous(limits = c(0,1)) +
    ylab("Object Expectation Accuracy") +
    xlab("Frequency") +
    ggtitle("Argument Structure: Active, Present") +
...
d_n_transf %>%
  ggplot(aes(x=freq_cat, y=pl_exp)) +
    theme_bw() +
    stat_smooth(aes(color=pos), method = 'glm', method.args = list(family = "binomial"), size = 0.5, alpha = 0.5) +
    stat_smooth(method = 'glm', method.args = list(family = "binomial"), linetype="dashed", size = 0.5) +
    geom_hline(yintercept=0.5, color="blue", alpha=0.5) +
    scale_y_continuous(limits = c(0,1)) +
    ylab("Object Expectation Accuracy") +
...
d_mod = d_n_transf %>%
  filter(test == "transf_simple") %>%
  glm(pl_exp ~ freq_cat * model, family = binomial, data = .)
summary(d_mod)

d_mod = d_n_transf %>%
  filter(test == "transf_mod") %>%
  glm(pl_exp ~ freq_cat * model, family = binomial, data = .)
summary(d_mod)
